...
def train():
    ...
    xin = tf.placeholder(dtype = tf.float32,
        # shape=[args.batch_size, args.height, args.width, args.channel],
        shape = [None, args.height, args.width, args.channel],
        name = "xin")
    yin = tf.placeholder(dtype = tf.int32,
        # shape=[args.eval_batch_size, args.num_labels],
    ...
    # compute graph
    tiny_densenet = TinyDenseNet(xin, args.num_labels, regularizer, 'tiny_densenet')
    logits = tiny_densenet.network2() 
    test_logits = tiny_densenet.network2(is_training = False, reuse = True)
        ...
        for epoch in range(sess.run(global_step) // train_batches_per_epoch, args.epoches):
            ...
            if (epoch) % args.eval_epoches == 0: 
                ...
                for _ in tqdm(range(eval_batches_per_epoch)):
                    ...
                    eval_image_batch = eval_image_batch / 255. - 0.5
                    eval_feed = {
