...
def get_weight_variable(name, shape, initializer, regularizer = None, lc = None):
    # lc means to loss collection.
    weights = tf.get_variable(
        name = name,
        shape = shape,
        initializer = initializer)
    # whether add to loss collection.
...
"""
def bn(name_scope, input_tensor, is_training = True, trainable = True, scale = True, 
        momentum = 0.999, epsilon = 1e-5, show_shape = True):
    ...
    with tf.variable_scope(name_scope):
        ...
        #     scale = scale,
        #     name = "bn")
        bn_out = tf.layers.batch_normalization(input_tensor, training = is_training)
        if show_shape:
            print(input_tensor.get_shape(), "-->", bn_out.get_shape(), name_scope + "/bn")
        return bn_out
"""

### Batch Normalization
def batch_norm(input_tensor, is_training=True, scope='batch_norm'):
    """"BatchNorm layer"""
    bn_out =  tf_contrib.layers.batch_norm(input_tensor, decay=0.9, epsilon=1e-05, 
    center=True, scale=True, updates_collections=None, is_training=is_training, scope=scope)
    return bn_out
    ...
    with tf.variable_scope(name_scope):
        if "max" == method.lower():
            pooled = tf.nn.max_pool(
                ...
                strides = [1, sh, sw, 1],
                padding = padding)
        else: # other now means to avg_pool
...
def fc(name_scope, input_tensor, num_output, trainable = True, weight_init = None, 
    ...
    input_shape = input_tensor.get_shape().as_list() # shape list
    size = 1
    ...
    with tf.variable_scope(name_scope):
        weights = tf.get_variable(
            ...
            initializer = weight_init,
            trainable = trainable)
        if None != regularizer: # need to be regularized
            tf.add_to_collection(lc, regularizer(weights))
...
def drop_out(name_scope, input_tensor, rate, is_training = True):
    '''dropout layer'''
    with tf.variable_scope(name_scope):
...
def conv_bn(name_scope, input_tensor, output_channels, ksize, strides = [1, 1],
    ...
    conv_layer = conv(name_scope, input_tensor, output_channels,
        ksize, strides, padding, bias, trainable,
        weight_init, bias_init, regularizer, lc)
    conv_layer_norm = batch_norm(conv_layer, is_training=is_training, scope = name_scope + "/bn")
...
# for feature fusion.
def fn(name_scope, tensor_h, tensor_l, is_training = False, weight_init = None, bias_init = tf.zeros_initializer(),
        regularizer = None, lc = "losses"):
    """
